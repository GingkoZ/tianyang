define(function(require, exports, module) {
    ...
    var LessonDashboard = Widget.extend({
        ...
        _afterLoadLesson: function(lessonId) {
            ...
            var self = this;
            this._counter = new Counter(self, type, this.get('courseId'), lessonId, this.get('watchLimit'));
        ...
        _onChangeLessonId: function(id) {
            ...
            $.get(this.get('courseUri') + '/lesson/' + id, function(lesson) {
                ...
                if (lesson.mediaSource == 'iframe') {
                    ...
                    } else if (lesson.mediaSource == 'smvp') {
                        var html = '<iframe name="smvp_frame" src="/smvp/video/'+lesson.courseId+'/lesson/' + lesson.id +'" style="position:absolute; left:0; top:0; height:100%; width:100%; border:0px;" scrolling="no"></iframe>';
                        $("#lesson-iframe-content").html(html);
                        $("#lesson-iframe-content").show();
                        
                        that.set("smvpFrameWindow",smvp_frame.window);
                    }else {
    ...
    var Counter = Class.create({
        initialize: function(dashboard, type, courseId, lessonId, watchLimit) {
        ...
        execute: function(){
			var posted = false;
			if(this.type="smvp"){
				posted = this.addSmvpPlayingCounter();
			}else if(this.player && this.type){
		...
		addSmvpPlayingCounter: function(){
            if( window.smvp_frame && window.smvp_frame.window.smvp){
                this.player = smvp_frame.window.smvp.players.get();
